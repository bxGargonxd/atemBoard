...
TimeoutSerial::TimeoutSerial() noexcept
    :
    io(),
    port(io),
    timer(io),
...
void TimeoutSerial::setTimeout(const boost::posix_time::time_duration& t) noexcept
{
    timeout = t;
}
...
{
    asio::write(port, asio::buffer(&data.at(0), data.size()));
}
...
std::vector<char> TimeoutSerial::read(size_t size)
{
    ...
    read(&result.at(0), size);//Fill it with values
    return result;
}
...
std::string TimeoutSerial::readStringUntil(const std::string& delim)
{
    ...
    for (;;)
    {
        ...
        case resultSuccess:
        {
            ...
            is.read(&result.at(0), bytesTransferred);//Fill values
...
void TimeoutSerial::timeoutExpired(const boost::system::error_code& error) noexcept
{
    if (!error && result == resultInProgress) result = resultTimeoutExpired;
}

void TimeoutSerial::readCompleted(const boost::system::error_code& error, const size_t bytesTransferred) noexcept
{
    if (!error)
    {
